...
void computeVertexNormals(TriangleMesh* tri_mesh) {
    ...
    for (i = 0; i < nt; ++i) {
        ...
        Vector3 p0p1; sub(p1, p0, &p0p1);
        Vector3 p0p2; sub(p2, p0, &p0p2);
	Vector3 cross; computeCrossProduct(p0p1, p0p2, &cross);
	float area; computeNorm(cross, &area); area /= 2;
...
void heatStep(TriangleMesh* tri_mesh) {
    ...
    int num_neighbors, vj;
    Vector3 l; l._x = 0; l._y = 0; l._z = 0;
    float lambda = 0.99;

    for(int i=0; i<tri_mesh->_number_vertices; i++){
	num_neighbors = getNumberAdjacentVertices(tri_mesh, i);
	for(int j=0; j<num_neighbors; j++){
	    vj = getAdjacentVertex(tri_mesh, i, j);
	    add(l, tri_mesh->_vertices[vj], &l);
	}
	mulAV((float)1/num_neighbors, l, &l);
...
void computeBoundaryEdges(TriangleMesh* mesh) {
    // Complete int flag;
    int tmp, flag;
    Pair edge1[3], edge2[3];
    mesh->_edges = (Pair*)malloc((mesh->_number_vertices*(mesh->_number_vertices-1)/2)*sizeof(Pair));
    mesh->_number_edges = 0;

    for(int i=0; i<mesh->_number_triangles; i++){
	edge1[0]._v0 = mesh->_triangles[i]._v0; edge1[0]._v1 = mesh->_triangles[i]._v1; 
	edge1[1]._v0 = mesh->_triangles[i]._v0; edge1[1]._v1 = mesh->_triangles[i]._v2; 
	edge1[2]._v0 = mesh->_triangles[i]._v1; edge1[2]._v1 = mesh->_triangles[i]._v2;

	for(int k=0; k<3; k++){
	    if(edge1[k]._v1 < edge1[k]._v0){
		tmp = edge1[k]._v0;
		edge1[k]._v0 = edge1[k]._v1;
		edge1[k]._v1 = tmp;
	    } 

	    flag = 0;
	    for(int j=0; j<mesh->_number_triangles; j++){
	        if(i == j) continue;
	        edge2[0]._v0 = mesh->_triangles[j]._v0; edge2[0]._v1 = mesh->_triangles[j]._v1; 
	        edge2[1]._v0 = mesh->_triangles[j]._v0; edge2[1]._v1 = mesh->_triangles[j]._v2; 
	        edge2[2]._v0 = mesh->_triangles[j]._v1; edge2[2]._v1 = mesh->_triangles[j]._v2; 

		for(int l=0; l<3; l++){
	            if(edge2[l]._v1 < edge2[l]._v0){
			tmp = edge2[l]._v0;
			edge2[l]._v0 = edge2[l]._v1;
			edge2[l]._v1 = tmp;
		    }
 
		    if(edge1[k]._v0==edge2[l]._v0 && edge1[k]._v1==edge2[l]._v1){
			flag = 1;
			break;
		    }
		}
	    ...
	    if(flag) continue;
	    mesh->_edges[mesh->_number_edges]._v0 = edge1[k]._v0;  
	    mesh->_edges[mesh->_number_edges]._v1 = edge1[k]._v1;
	    mesh->_number_edges++;
	}
    }
}
