...
static void myRotatef(GLfloat theta, GLfloat kx, GLfloat ky, GLfloat kz) {
    // Complete
    GLfloat c = cos(theta/180.0f*_PI);
    GLfloat s = sin(theta/180.0f*_PI);
    const GLfloat m[16] = {
...
static void myPerspective(GLfloat fovy, GLfloat aspect, GLfloat n, GLfloat f) {
    // Complete
    GLfloat t = tan(fovy/180.0f*_PI/2);
    GLfloat left = -t*n*aspect;
    GLfloat right = t*n*aspect;
    GLfloat bottom = -t*n;
    GLfloat top = t*n;
    const GLfloat m[16] = {
...
static void drawMultipleCopies(int n){
    if(n==0) return;
    if(n > 5) n = 5;
    glPushMatrix();			//Save the current coordinates
    if(g_func_mode == OPENGL){
        glScalef(0.5f, 0.5f, 0.5f);	//Scale down
        glTranslatef(-2.0f, -3.0f, 0);	//Translate
    }else{
	myScalef(0.5f, 0.5f, 0.5f);
	myTranslatef(-2.0f, -3.0f, 0);
    }
    drawIcosahedron();			//Draw the left one
    drawMultipleCopies(n-1);		//Recursive call to the left one
    if(g_func_mode == OPENGL){
	glTranslatef(4.0f, 0, 0);	//Translate
    }else{
	myTranslatef(4.0f, 0, 0);
    }
    drawIcosahedron();			//Draw the right one
    drawMultipleCopies(n-1);		//Recursive call to the right one
    glPopMatrix(); 			//Restore the saved coordinates
}
